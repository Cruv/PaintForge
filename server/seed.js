...
// Same seed data as src/db/seed.ts — duplicated here since
// the server can't import TypeScript modules at runtime.
const SEED_PAINTS = [
...
export function seedIfNeeded() {
  ...
  if (row && row.count > 0) {
    console.log(`Seed data already present (${row.count} paints)`);
    return;
  }

  const now = new Date().toISOString();
  for (const paint of SEED_PAINTS) {
    const entity = { ...paint, created: now, updated: now };
    run(
      'INSERT OR REPLACE INTO entities (id, type, name, data, created, updated) VALUES (?, ?, ?, ?, ?, ?)',
      [entity.id, entity.type, getEntityName(entity), JSON.stringify(entity), entity.created, entity.updated]
    );
  }
  console.log(`Seeded ${SEED_PAINTS.length} paints`);
}
